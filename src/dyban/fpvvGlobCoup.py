...
class FpVVglobCoupled(BayesianPieceWiseLinearRegression):
  ...
  def fit(self):
    ...
    for it in tqdm(range(self.num_iter)):
      ...
      # Append the sample
      beta.append(sample)
      # dont need to pad the full parents model (unless you specify the network)
      ...
      ################ Sample mu* without a change on the parent set (allways accepted? TODO ask marco)
      sample, _, _ = vvMuSampler(muVector[it], changePoints, X, y,
        sigma_sqr_vector[it + 1], lambda_sqr[it + 1])
      
      muVector.append(sample) # append the sample mu
